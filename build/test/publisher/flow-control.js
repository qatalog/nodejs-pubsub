...
const fc = require("../../src/publisher/flow-control");
(0, mocha_1.describe)('Flow Controller', () => {
    const optionsDefault = {
        maxOutstandingMessages: 5,
        maxOutstandingBytes: 100,
    };
    (0, mocha_1.it)('does basic bookkeeping correctly', async () => {
    ...
    function requestQueue(flow) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return flow.requests;
    }
    (0, mocha_1.it)('queues up a promise when bytes are exceeded', async () => {
        const flow = new fc.FlowControl(optionsDefault);
    ...
    (0, mocha_1.it)('queues up a promise when messages are exceeded', async () => {
        const flow = new fc.FlowControl(optionsDefault);
    ...
    (0, mocha_1.it)('releases a publisher when space is freed', async () => {
        const flow = new fc.FlowControl(optionsDefault);
        const promise = flow.willSend(1000, 1);
        const requests = requestQueue(flow);
        assert.strictEqual(requests.length, 1);
    ...
    (0, mocha_1.it)('releases a publisher only when enough space is freed', async () => {
        const flow = new fc.FlowControl(optionsDefault);
    ...
    (0, mocha_1.it)('calculates with wouldExceed correctly', () => {
    ...
    (0, mocha_1.it)('sets options after the fact', () => {
        const flowPause = new fc.FlowControl(optionsDefault);
        const newOptions = {
    ...
    (0, mocha_1.it)('does not allow nonsensical option values', () => {
        const flowPause = new fc.FlowControl(optionsDefault);
        const newOptions = {
